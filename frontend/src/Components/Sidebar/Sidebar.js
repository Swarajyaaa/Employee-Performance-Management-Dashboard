    ...
    return (
      <div className='hidden z-20 px-[1.5vw] py-[1vw] fixed top-0 left-0 h-screen bg-white border-r w-[16.87vw] md:block'>
        ...
        <NavLink to="/projects" className={({isActive} ) => isActive ? normalClasses + activeClasses : normalClasses}><img src="" className='w-[1.5vw]' alt="" /><span className='text-[1vw]'>All Projects</span></NavLink>
          <NavLink to="/" className={({isActive} ) => isActive ? normalClasses + activeClasses : normalClasses}><img src="" className='w-[1.5vw]' alt="" /><span className='text-[1vw]'>Dashboard</span></NavLink>
          {isAdmin && <NavLink to="/departments" className={({isActive} ) => isActive ? normalClasses + activeClasses : normalClasses}><img src="" className=' w-[1.5vw]' alt="" /><span className='text-[1vw]'>Departments</span></NavLink>}
          {isAdmin && <NavLink to="/salary" className={({isActive} ) => isActive ? normalClasses + activeClasses : normalClasses}><img src="" className='w-[1.5vw]' alt="" /><span className='text-[1vw]'>Pay Salary</span></NavLink>}
          <button onClick={() => {

          }} className={normalClasses}><img src="" className='w-[1.5vw]' alt="" /><span className='text-[1vw]'>Logout</span></button>
        </div>
