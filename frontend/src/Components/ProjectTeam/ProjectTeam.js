...
function ProjectTeam({id,setIsOpen,setClickedUserObject}) {
  ...
  return (
    <div className="bg-white shadow-md rounded-md w-[300px] pb-4">
      ...
      <div className="projectMembers p-4 space-y-2 h-[78vh] overflow-y-auto">
         {allMembers.map(members => <ProjectMember setClickedUserObject = {setClickedUserObject}  setIsOpen={setIsOpen} key={members.id} member={members} />)}
      </div>
    </div>
  );
}
